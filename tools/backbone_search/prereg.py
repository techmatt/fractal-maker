...
def main():
    ...
    doc = {
      ...
      "round_2_rule": {
        "advance": "the 2 non-control arms with the highest pooled PRIMARY AUC(>=3) in "
                   "round 1, plus a 3rd iff its point estimate is within 0.005 of the 2nd "
                   "(so a near-tie is not resolved by a coin-flip at the cut). The control "
                   "always advances.",
        "seeds": [0, 1, 2],
        ...
        "reported": "band (min, mean, max) per arm; the control's own band is the noise "
                    "floor every delta is read against.",
        "win_definition": "An arm is called a WIN over the control only if BOTH hold: (a) "
                          "its 3-seed minimum on the pooled PRIMARY AUC(>=3) exceeds the "
                          "control's 3-seed MAXIMUM, and (b) the paired cluster-bootstrap "
                          "CI of the delta excludes 0 for every matched seed pair (arm "
                          "seed i vs control seed i). The two answer different questions — "
                          "(a) is seed variance, (b) is sampling variance on 2,190 rows — "
                          "and an arm that clears one but not the other is reported as a "
                          "TIE with both numbers shown."},
      "cost_projection": {
