...
| `v11/` | 7 | **The v11 training generation.** `build_manifest.py` (a FRESH rebuild with randomized location-grouped splits — two `eval_role`s, `instrument` unbiased and `holdout` biased-by-design), `build_plan.py` (the dictated 32-tile independent-draw recipe), `render_cache.py` (the chunked supervisor) and `verify_cache.py`. The prep harness stays: `parity_crop_mode.py` (the functional-parity read that adopted `field_ss 2`) and `measure_crop_cost.py`. Tests: `test_v11_build.py` (default lane — the committed records against the modules that wrote them) and `test_crop_batch.py` (`slow`: both fan-out modes, replay, containment). The executor is Rust (`src/crop_batch.rs`); the manifest/plan/cache are **bulk**, out-of-tree, with only `data/v11/{build_record,aug_recipe,colormaps}.json` committed. | **live** | — |
