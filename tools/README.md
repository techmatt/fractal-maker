...
555 tracked files, 521 of them `.py`, across 35 subdirectories. This file is the entry
...
**File counts re-derived 2026-08-07 at this commit** (the 2026-07-31 header said 413/381,
...
| `emission/` | 28 | **Diversity-aware emission v1**: `cells.py` (joint-count cells + the target measure DERIVED from `scoring/release_mix.RATIO`), `descriptor.py` (intake → namespaced cross-ledger union → partition-keyed morph cluster), `palette_deficit.py`, `pool.py`, `selection.py`, `release_record.py`, driven by `build_emission_diversity_v1.py`. `attempt_budget.py` (2026-08-09) is THE colorize VOLUME rule: attempts split head-first at `floors.ATTEMPT_MULTIPLIER` (4) x that head's release slots — both heads scaled down proportionally when `--max-attempts` cannot cover the pair, never one starved — then per partition by the same `release_mix` apportionment the release slots use, filled in rank order. It replaced the deficit model as the volume rule (that model spread over a style axis with one smooth style against N strange ones, so smooth drew 3 of 30 attempts against 6 slots); the deficit model still picks the (flavor, style) pair INSIDE the paying head. `ranked_intake.py` (2026-08-09) is the READ-TIME intake: guard ∧ distinct ∧ raw P(>=3) >= the 0.20 junk floor (floor-admit sources bypass), ranked best-first per partition — NO decode-version predicate and no stored-`decoded_class` q3 gate. `floors.py` (2026-08-04) is THE stage-2 cut owner; since 2026-08-09 its four stamped floors (wallpaper pool 0.75 / release 0.90, mining pool 0.25 / release 0.50) are **ANNOTATION-ONLY** and the ONE enforcing cut is `JUNK_FLOOR` = 0.20 at the colorize-pool draw (read by `ranked_intake` and `mining/deploy_tail`), with `THIN_SUPPLY_DIVISOR` = 4 and `CLUSTER_CAP` = 2 declared beside it. Each retired floor keeps its head VERSION stamp and still refuses when the live pin disagrees — an annotation on the wrong scale is as unreadable as a gate on one; `test_floors_one_source.py` scans the stage-2 surface for a re-typed literal (there were six copies of four numbers). `selection.py` holds both release rules: `rank_select` (live — top-N by the head's own score under the partition slot / thin-supply / morph-cluster caps) and `greedy_select` (retired from the live path, kept for the v1 reconstruction). `ledger_rescore.py` (2026-08-04) brings the seven intake ledgers current under the active head — a sibling `<stem>.rescored_<version>.jsonl` that `descriptor.resolve_rows` overlays, never an in-place edit; hand-run at a flip (`classifier_retrain_protocol.md` §5). Six of the seven carry that sibling; `classic_phoenix` is in the list so the pass **verifies** it is current rather than rescoring it — it re-mints under the live pins on every run, so it arrives current. Its `rescored.jsonl` is `classic_phoenix_supply`'s own resume state and is deliberately NOT this convention (`descriptor.RESCORE_SUFFIX_FMT`). | **live** | 17 / 13 / 6 / 14 |
