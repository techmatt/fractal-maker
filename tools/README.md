...
| `v11/` | 3 | **v11 prep — the extended-field crop executor's harness.** Not a training generation: no manifest, no plan, no cache. `parity_crop_mode.py` (the functional-parity read of `crop-batch` against `v4-render-batch` at identity geometry, across families, fp32) and `measure_crop_cost.py` (run-order cost + disk sizing for the rebuild), plus `test_crop_batch.py` (`slow`: seed→tile determinism, manifest replay, and the containment error). All three are hand-run entry points writing only `scratch/`, so all three are **dead by both methods by construction** (blind spot 1). The executor itself is Rust (`src/crop_batch.rs`). | **live** (staged) | 2 / 0 / 0 / 0 |
