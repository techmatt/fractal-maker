...
| `mining/` | 12 | The strange/render-mode quality gate: `mining_pins.py` (the TORCH-FREE pin block — ckpt, threshold, derived version tag) + `mining_gate.py` (the pinned `mining_v1`, re-exporting it), `score_lib.py` (the v3 deploy-transform scorer, 28 importers), `dedup.py`, `tail_alloc.py`, `deploy_tail.py`. | **live** | 10 / 7 / 2 / 7 |
