#!/usr/bin/env python
"""The v8 t_good derivation ⟷ production table agreement gate.

`data/v8/t_good_derivation.json` is the DERIVATION and `production_seeder.T_GOOD_OVERRIDES`
is the ADOPTED copy. Two copies of the same numbers drift; this holds them equal. It also
pins the thing the numbers alone cannot say: which partitions are UNCALIBRATED (running at
the baseline without ever having been derived) vs DERIVED-at-0.50, because in a config file
those are the same character sequence.
...
import derive_t_good_v8 as dv8  # noqa: E402

DERIVATION = ROOT / "data" / "v8" / "t_good_derivation.json"
...
def test_adopted_table_matches_the_derivation_artifact():
    ...
    assert doc["adopted"] == {k: float(v) for k, v in ps.T_GOOD_OVERRIDES.items()}, (
        ...
        f"{doc['adopted']} — re-run tools/v8/derive_t_good_v8.py and mirror the table")
...
def test_derived_partitions_carry_the_supply_weighted_objective():
    # The split objective is the point of the v8 derivation: precision where supply is
...
def test_class4_has_no_per_family_threshold():
    # v8 is q4-capable; the derivation deliberately does NOT calibrate the third cutpoint.
    doc = _doc()
...
def test_fbeta_matches_the_closed_form():
    ...
    for beta in (0.5, 1.0, 2.0):
        b2 = beta * beta
        assert abs(dv8.fbeta(p, r, beta) - (1 + b2) * p * r / (b2 * p + r)) < 1e-12
    assert dv8.fbeta(0.0, 0.0, 2.0) == 0.0


def test_derivation_is_stamped_with_the_model_it_came_from():
    doc = _doc()
    assert doc["model"] == "v8"
    assert doc["eval_slice"] == "data/v8/eval_scores_v8.jsonl"
