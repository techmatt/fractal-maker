...
THE BUILDER HALF IS GONE (2026-08-10). `main()`, its ten gates and `load_prior_loc_ids`
were deleted with the closure sweep: nothing invoked them, and a re-run would have
rewritten the tracked `data/v8/{manifest.jsonl,eval_slice.jsonl,build_metadata.json}`
under a population the corpus no longer has. Those three records STAY, frozen, as the
description of what v8 was built from; the prose above describes THAT build and is
history, not a runnable recipe. The `OUT`/`EVAL_OUT`/`META_OUT` names below are kept as
the record's addresses (v10/v11 gates read them), not as write targets.

WHAT SURVIVES IS A LIBRARY, and it is live: the neighborhood-clustering predicate
(`UF`/`cluster`/`C_TOL_FRAC`), the corpus load + label overlay (`load_all_labeled`),
classification (`classify_batch`/`classify_location`/`ftype_of`), grouping and the
group-derived split (`assign_groups`/`assign_split_by_group`/`straddle_report`), the
row/identity spellings (`row_of`/`ident`/`ident_of_loc`) and the census constants. The
LIVE `tools/v11/build_manifest.py` imports it, as do `tools/v10/build_manifest.py` and
their tests — the vN chain is a copy-forward everywhere else, and this is the one place it
is a real import. Deleting the module would take v11's builder with it.
"""
