#!/usr/bin/env python
r"""build_combined_label_sheet.py — serve several registered batches as ONE blind sitting.
...
about which batch or which cell a row came from.
  ...
  uv run python tools/corpus/build_combined_label_sheet.py merge-dryrun  # export -> 3 batches
"""
...
# =========================================================================== #
def sheet_dir() -> Path:
    return Path(cc.batch_dir(SHEET_ID))
...
def opaque_id(slot: int, batch: str, image_id: str) -> str:
    """`qc<slot>_<hash>` — slot is presentation position (assigned POST-shuffle, so it orders
    the sitting and nothing else); the hash makes the id a stable function of the row it
    stands for, so a rebuild of the sheet reproduces the same ids."""
    h = hashlib.blake2b(f"{ID_SALT}|{batch}|{image_id}".encode(), digest_size=4).hexdigest()
    return f"qc{slot:04d}_{h}"


def ordered_union(pairs):
    ...
    jitter = {c: random.Random(f"{UNION_SEED}|{c}").random() for c in cells}
...
def stage_verify(args) -> int:
    ...
    emit(f"=== combined q4 label sheet — SOURCE verification ({STAMP}) ===")
    pairs = load_sources()
    emit(f"union: {len(pairs)} rows over {len(SOURCES)} batches")
    ...
    for d in sorted(os.listdir(cc.BATCHES_DIR)):
        ...
        if d in SOURCES or not os.path.exists(p):
            continue
    ...
    _write_log(L, "verify_sources.txt")
    return 0 if ok else 1


def _write_log(lines, name):
    p = paths.scratch("q4_combined_sheet", name)
...
def stage_build(args) -> int:
    pairs = load_sources()
    order = ordered_union(pairs)
    sd = sheet_dir()

    sheet_rows, route = [], {}
    for slot, (cell, batch, row) in enumerate(order):
        oid = opaque_id(slot, batch, row["image_id"])
    ...
    bj = {
        ...
        "order_rule": ("seeded shuffle over the union, then stratified round-robin over every "
                       ...
                       "prefix"),
        "id_salt": ID_SALT,
        "id_rule": "qc<slot>_<blake2b(salt|batch|image_id)>; slot assigned POST-shuffle",
    ...
    if not args.apply:
        head = [f"{b.split('_q4_')[-1]}|{f}" for (b, f), _, _ in order[:12]]
    ...
    crops_out, vivid_out = Path(cc.crops_dir(SHEET_ID)), Path(cc.vivid_dir(SHEET_ID))
...
def stage_check(args) -> int:
    ...
    sd = sheet_dir()
    emit(f"=== combined q4 label sheet — BUILT-BYTES acceptance ({STAMP}) ===")
    emit(f"[{SHEET_ID}]")
    if not (sd / SHEET_MANIFEST).exists():
        check("sheet built", False, str(sd))
        _write_log(L, "check_sheet.txt")
        return 1
    ...
    src = {b: cc.read_jsonl(os.path.join(cc.batch_dir(b), "images.jsonl")) for b in SOURCES}
    src_by_id = {b: {r["image_id"]: r for r in rs} for b, rs in src.items()}

    check("870 rows served", len(rows) == sum(len(v) for v in src.values()),
          f"{len(rows)} vs {sum(len(v) for v in src.values())}")
    ...
    check("no source batch_id appears in the served bytes",
          not any(b in served_text for b in SOURCES))
    ...
    check("ids are opaque `qc<slot>_<hash>` and unique",
          len({r["image_id"] for r in rows}) == len(rows)
          and all(r["image_id"].startswith("qc") and len(r["image_id"]) == 15 for r in rows))
    check("no served id contains a source image_id",
          not any(r["image_id"][7:] in {i for m in src_by_id.values() for i in m}
                  for r in rows))
    # the id is assigned POST-shuffle: slot order == file order, and nothing else.
    check("slot numbering follows the served order",
          all(int(r["image_id"][2:6]) == i for i, r in enumerate(rows)))
    ...
    check("every source row appears exactly once",
          all(per_src[b] == len(src[b]) for b in SOURCES), str(dict(per_src)))
    ...
    # Run-length caps, one step above the MEASURED spread. Over six union seeds the deal gave
    # same-source run 3 every time and same-family run 3-4 (the cell SEQUENCE is near
    # seed-independent — the shuffle permutes rows within a cell, not the cells themselves), so
    # 4/5 leave headroom for a reseed without going slack. The load-bearing balance invariant is
    # the +/-1 prefix bound above; these two are the "no stretch is single-X" statement itself.
    check("no stretch of the sitting is single-source", runs_src <= 4, f"longest run {runs_src}")
    check("no stretch of the sitting is single-family", runs_fam <= 5, f"longest run {runs_fam}")
    ...
    for r in rows:
        for kind in ("crops", "vivid"):
            url = f"/data/label_corpus/batches/{SHEET_ID}/{kind}/{r['image_id']}.jpg"
            if not os.path.exists(handler.translate_path(url)):
    ...
    for r in sample:
        ...
        for kind, d_s, d_b in (("crops", cc.crops_dir(SHEET_ID), cc.crops_dir(b)),
                               ("vivid", cc.vivid_dir(SHEET_ID), cc.vivid_dir(b))):
...
def stage_merge_dryrun(args) -> int:
    """Prove the one-file export routes: synthesize a full 870-row labels.json, merge it into a
    COPY of the corpus, and assert per-batch counts and an untouched amendment overlay."""
    import tempfile
    L, ok = [], True
    ...
    route = ms.load_route(str(sd / ROUTE_FILE))
    rows = cc.read_jsonl(str(sd / SHEET_MANIFEST))
    emit(f"=== combined export -> three batches — SANDBOX dry run ({STAMP}) ===")
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        check("every exported id routes to a registered source batch",
              set(per_batch) == set(SOURCES), str(sorted(per_batch)))
        ...
        live_labeled = sum(1 for b in SOURCES
                           for r in cc.read_jsonl(os.path.join(cc.batch_dir(b), "images.jsonl"))
...
def main(argv=None) -> int:
    ...
    b.add_argument("--apply", action="store_true", help="write (default: dry run)")
    b.set_defaults(fn=stage_build)
    sub.add_parser("check").set_defaults(fn=stage_check)
    sub.add_parser("merge-dryrun").set_defaults(fn=stage_merge_dryrun)
