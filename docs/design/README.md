...
| [deferred_recalibration.md](deferred_recalibration.md) | Three recalibrations still deliberately unbuilt (ranker growth, location blind reads, mining-head calibration), the release-review gate that unparks them, plus the record of the fourth: the location-head retrain, flipped to v10 on 2026-08-02 with its whole threshold cluster. |
...
| [release_concurrency.md](release_concurrency.md) | Process concurrency on the emission release render pass (`release_pass.py`): the workers-render/parent-writes rule that makes concurrent records *identical* to serial ones rather than equivalent, the two temp-name collisions N processes expose, the measured (workers × engine-threads) sweep with full-res byte parity on all 8 arms, why dividing the box among the workers LOSES to nominal oversubscription at every N, and why the ceiling is ~1.5× rather than the projected 2.5–3.5× (memory-bandwidth-bound tail + a critical-path phoenix row). |
