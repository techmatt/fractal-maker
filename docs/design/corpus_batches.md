...
| `2026-08-11_render_mode_baserate_audit_v1` (sheet F, 200) | The **base-rate audit** (state §OPEN-1). **Score-unconditioned DRAW, anchored PAGE** — and the two halves land on opposite sides: no mining head touches the selection (sheet E's population *imported*, flat mode apportionment, pool palette, near-dup ties broken by draw order), but v3's tier is prefilled and the page is sorted by its readout, so the labels are anchored ⇒ **train-side, protocol §2b**, and it never joins D/E. 200 rows over 131 locations, mode mix flat at 14–15 across the 14 active modes. **Rates off it are CEILINGS**; sheet E is the unanchored bound on the same draw rule and the pair is what the audit reads. **LABELED 2026-08-11**, tiers {1:93, 2:88, 3:19} — 53.5% ≥2 and 9.5% ≥3 against sheet E's blind 50.7% / 4.0%, correction rate 176/200 = 88.0%. It is what the mining gate was moved on: the isotonic crossover of `1[label ≥ 2]` against v3's `p_ge3` is 0.0949 (`data/render_mode_head/v3/baserate_audit_2026-08-11.json`). Still not in `near_dup_groups.BATCHES` — pooling is a retrain decision and was explicitly out of the audit's scope. |
